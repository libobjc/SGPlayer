...
@property (nonatomic, strong) NSMutableArray <__kindof SGFrame2 *> * frames;

@property (nonatomic, assign) BOOL destoryToken;

@end

@implementation SGFrameQueue2
...
- (void)putSortFrame:(__kindof SGFrame2 *)frame
{
    ...
    if (self.frames.count > 0) {
        for (int i = (int)self.frames.count - 1; i >= 0; i--) {
            SGFrame2 * obj = [self.frames objectAtIndex:i];
...
- (__kindof SGFrame2 *)getFrameSync
{
    ...
    SGFrame2 * frame = self.frames.firstObject;
...
- (__kindof SGFrame2 *)getFrameAsyncPosistion:(NSTimeInterval)position discardFrames:(NSMutableArray <__kindof SGFrame2 *> **)discardFrames
{
    ...
    for (SGFrame2 * obj in self.frames) {
